import { useMemo, useState } from "react";
import { NavLink, useLocation, useNavigate } from "react-router-dom";
import {
  Home,
  CalendarDays,
  CalendarRange,
  Users,
  UserCog,
  Settings,
  Building2,
  LogOut,
  HelpCircle,
  ClipboardCheck,
  MessageSquare,
  UserPlus,
  Lightbulb,
  BarChart3,
  ClipboardList,
  History,
  ChevronDown,
  Menu,
  type LucideIcon,
} from "lucide-react";
import { cn } from "@/lib/utils";
import { useAuth } from "@/context/AuthContext";

export type SidebarLink = {
  label: string;
  href: string;
  icon: LucideIcon;
  description: string;
};

export type SidebarGroup = {
  id: string;
  label: string;
  hint: string;
  icon: LucideIcon;
  links: SidebarLink[];
  includeLogout?: boolean;
};

export const dashboardLink: SidebarLink = {
  label: "Dashboard",
  href: "/dashboard",
  icon: Home,
  description: "Overview and important updates.",
};

export const sidebarGroups: SidebarGroup[] = [
  {
    id: "organizations",
    label: "Organizations",
    hint: "Clubs and leadership",
    icon: Building2,
    links: [
      {
        label: "Clubs",
        href: "/clubs",
        icon: Building2,
        description: "Club records and management.",
      },
      {
        label: "Prospects",
        href: "/prospects",
        icon: Lightbulb,
        description: "Prospective clubs.",
      },
      {
        label: "Officers",
        href: "/officers",
        icon: Users,
        description: "Officer management.",
      },
      {
        label: "Add Members",
        href: "/members/add",
        icon: UserPlus,
        description: "Add members quickly.",
      },
    ],
  },
  {
    id: "programming",
    label: "Programming",
    hint: "Events and forms",
    icon: CalendarDays,
    links: [
      {
        label: "Events",
        href: "/events",
        icon: CalendarDays,
        description: "Manage events.",
      },
      {
        label: "Calendar",
        href: "/calendar",
        icon: CalendarRange,
        description: "View schedules.",
      },
      {
        label: "Forms",
        href: "/forms",
        icon: ClipboardList,
        description: "Forms and responses.",
      },
    ],
  },
  {
    id: "operations",
    label: "Operations",
    hint: "Daily review and communication",
    icon: ClipboardCheck,
    links: [
      {
        label: "Approvals",
        href: "/approvals",
        icon: ClipboardCheck,
        description: "Review requests and approvals.",
      },
      {
        label: "Tasks",
        href: "/tasks",
        icon: ClipboardList,
        description: "Tasks and follow-ups.",
      },
      {
        label: "Messaging",
        href: "/messaging",
        icon: MessageSquare,
        description: "Messages and conversations.",
      },
      {
        label: "Audit Log",
        href: "/audit-log",
        icon: History,
        description: "History of admin actions.",
      },
    ],
  },
  {
    id: "administration",
    label: "Administration",
    hint: "Account and settings",
    icon: Home,
    links: [
      {
        label: "User Management",
        href: "/users",
        icon: UserCog,
        description: "Manage user access.",
      },
      {
        label: "Analytics",
        href: "/analytics",
        icon: BarChart3,
        description: "Reports and trends.",
      },
      {
        label: "Settings",
        href: "/settings",
        icon: Settings,
        description: "Workspace settings.",
      },
      {
        label: "Support",
        href: "/support",
        icon: HelpCircle,
        description: "Help and support.",
      },
    ],
  },
];

export const flatSidebarLinks: SidebarLink[] = [
  dashboardLink,
  ...sidebarGroups.flatMap((group) => group.links),
];

export const primaryLinks: SidebarLink[] = [dashboardLink];
export const manageLinks: SidebarLink[] = sidebarGroups
  .filter((group) => group.id !== "administration")
  .flatMap((group) => group.links);
export const accountLinks: SidebarLink[] =
  sidebarGroups.find((group) => group.id === "administration")?.links ?? [];

const sidebarGroupOrder = ["organizations", "programming", "operations", "administration"] as const;
const orderedSidebarGroups = sidebarGroupOrder
  .map((groupId) => sidebarGroups.find((group) => group.id === groupId))
  .filter((group): group is SidebarGroup => Boolean(group));

export function matchesSidebarPath(pathname: string, href: string) {
  return pathname === href || pathname.startsWith(`${href}/`);
}

interface SidebarProps {
  className?: string;
  open: boolean;
  setOpen: React.Dispatch<React.SetStateAction<boolean>>;
}

function Sidebar({ className, open, setOpen }: SidebarProps) {
  const location = useLocation();
  const navigate = useNavigate();
  const { signOut } = useAuth();
  const initialExpandedGroups = useMemo(
    () =>
      Object.fromEntries(
        orderedSidebarGroups.map((group) => [
          group.id,
          group.links.some((link) => matchesSidebarPath(location.pathname, link.href)) || group.id === "operations",
        ]),
      ),
    [location.pathname],
  );
  const [expandedGroups, setExpandedGroups] = useState<Record<string, boolean>>(initialExpandedGroups);

  const handleSignOut = async () => {
    await signOut();
    navigate("/login");
  };

  return (
    <aside className={cn("hidden md:block w-20", className)}>
      {open ? (
        <button
          type="button"
          aria-label="Close sidebar"
          onClick={() => setOpen(false)}
          className="fixed inset-0 left-20 z-40 bg-slate-950/10 backdrop-blur-[3px]"
        />
      ) : null}

      <nav
        className={cn(
          "fixed left-0 top-0 z-50 flex h-screen flex-col border-r border-slate-300 bg-[#f7f7f5] text-slate-700 transition-[width,box-shadow] duration-200",
          open ? "w-64 shadow-2xl" : "w-20",
        )}
      >
        <div className={cn("border-b border-slate-300 px-4 py-4", !open && "px-3")}>
          <div className={cn("flex items-center", open ? "justify-between gap-3" : "flex-col gap-3")}>
            <button
              type="button"
              onClick={() => setOpen((current) => !current)}
              className={cn(
                "grid h-10 w-10 shrink-0 place-content-center rounded-xl border border-blue-600 bg-blue-600 text-white transition-colors hover:bg-blue-700 hover:border-blue-700 hover:text-white",
                open ? "order-2" : "mb-1",
              )}
              aria-label={open ? "Close sidebar" : "Open sidebar"}
            >
              <Menu className="h-5 w-5" />
            </button>

            <div className={cn("flex items-center gap-3", !open && "flex-col")}>
              <Logo />
              {open ? (
                <div className="min-w-0">
                  <p className="truncate text-base font-semibold text-slate-900">Connect Camp</p>
                  <p className="text-xs text-slate-500">Admin Dashboard</p>
                </div>
              ) : null}
            </div>
          </div>
        </div>

        <div className="flex-1 overflow-y-auto py-2">
          {open ? (
            <div className="space-y-5">
              <div>
                <SectionLabel label="Home" />
                <div className="mt-1">
                  <NavItem link={dashboardLink} open onNavigate={() => setOpen(false)} />
                </div>
              </div>

              {orderedSidebarGroups.map((group) => (
                <DropdownSection
                  key={group.id}
                  group={group}
                  expanded={expandedGroups[group.id] ?? false}
                  onExpandedChange={(value) =>
                    setExpandedGroups((current) => ({ ...current, [group.id]: value }))
                  }
                  onNavigate={() => setOpen(false)}
                />
              ))}

              <div className="border-t border-slate-300 px-2 pt-2">
                <SignOutButton open onClick={handleSignOut} />
              </div>
            </div>
          ) : null}
        </div>
      </nav>
    </aside>
  );
}

function NavItem({
  link,
  open,
  onNavigate,
}: {
  link: SidebarLink;
  open: boolean;
  onNavigate?: () => void;
}) {
  const location = useLocation();
  const Icon = link.icon;
  const isActive = matchesSidebarPath(location.pathname, link.href);

  return (
    <NavLink
      to={link.href}
      className={({ isActive: routeActive }) =>
        cn(
          "flex items-center gap-3 border-l-2 border-transparent px-4 py-2.5 text-sm transition-colors",
          !open && "justify-center px-0",
          routeActive || isActive
            ? "border-slate-900 bg-slate-200 font-semibold text-slate-950"
            : "text-slate-600 hover:bg-slate-100 hover:text-slate-900",
        )
      }
      title={!open ? link.label : undefined}
      aria-label={link.label}
      onClick={onNavigate}
    >
      <Icon className="h-4 w-4 shrink-0" />
      {open ? <span className="truncate">{link.label}</span> : null}
    </NavLink>
  );
}

function SignOutButton({
  open,
  onClick,
}: {
  open: boolean;
  onClick: () => Promise<void>;
}) {
  return (
    <button
      type="button"
      onClick={onClick}
      className={cn(
        "flex w-full items-center gap-3 border-l-2 border-transparent px-4 py-2.5 text-sm text-slate-600 transition-colors hover:bg-slate-100 hover:text-slate-900",
        !open && "justify-center px-0",
      )}
      title={!open ? "Sign out" : undefined}
      aria-label="Sign out"
    >
      <LogOut className="h-4 w-4 shrink-0" />
      {open ? <span>Sign out</span> : null}
    </button>
  );
}

function DropdownSection({
  group,
  expanded,
  onExpandedChange,
  onNavigate,
}: {
  group: SidebarGroup;
  expanded: boolean;
  onExpandedChange: (value: boolean) => void;
  onNavigate: () => void;
}) {
  const location = useLocation();
  const hasActiveChild = group.links.some((link) => matchesSidebarPath(location.pathname, link.href));

  return (
    <section>
      <SectionLabel label={group.id === "administration" ? "Admin" : group.label} />
      <button
        type="button"
        onClick={() => onExpandedChange(!expanded)}
        className={cn(
          "mt-1 flex h-14 w-full items-center justify-between border-y border-slate-300 bg-transparent px-4 text-left transition-colors",
          hasActiveChild ? "text-slate-950" : "text-slate-700 hover:bg-slate-100",
        )}
      >
        <span className="flex items-center gap-3">
          <group.icon className="h-5 w-5 shrink-0" />
          <span className="text-sm font-medium">
            {group.id === "organizations" ? "Clubs" : group.id === "administration" ? "Admin" : group.label}
          </span>
        </span>
        <ChevronDown className={cn("h-4 w-4 transition-transform", expanded && "rotate-180")} />
      </button>

      {expanded ? (
        <div className="border-b border-slate-300 bg-white/90 py-1">
          {group.links.map((link) => (
            <NavItem key={link.href} link={link} open onNavigate={onNavigate} />
          ))}
        </div>
      ) : null}
    </section>
  );
}

function SectionLabel({ label }: { label: string }) {
  return (
    <p className="px-4 text-[11px] font-semibold uppercase tracking-[0.18em] text-slate-400">
      {label}
    </p>
  );
}

function Logo() {
  return (
    <div className="grid h-10 w-10 shrink-0 place-content-center bg-slate-900 text-white">
      <svg viewBox="0 0 128 128" className="h-5 w-5" aria-hidden="true" focusable="false">
        <rect width="128" height="128" rx="18" fill="currentColor" />
        <path
          fill="#0f172a"
          d="M64 18c-25.4 0-46 20.6-46 46s20.6 46 46 46c11.2 0 21.8-4 30.1-11.4l-14-16.8A23 23 0 0 1 64 85c-13 0-23.6-10.6-23.6-23.7S51 37.5 64 37.5c7.7 0 14.7 3.7 19.2 9.6l13.9-16.7C88.7 22 76.8 18 64 18Z"
        />
        <circle cx="64" cy="61.3" r="11" fill="#fff" />
      </svg>
    </div>
  );
}

export default Sidebar;
